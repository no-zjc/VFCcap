...
class COAHA:
    ...
    def __init__(self, cfg, gt_dict):
        ...
        self.frequent_words_action = ['a', 'is', 'a', 'face', 's', 'are', 'two', 'up', 'rope', 'the', 'car', 'bowl',
                                      'flour', 'ball',
                                      'bread', 'face', 'with', 'his', 'her', 'dog', 'bike']

        self.action_add = []
        # f = open(os.getcwd() + "/Metric/THVC/" + 'action_list', 'r')
        f = open(os.getcwd() + '/action_list', 'r')
        ...
        # 根据设置阈值将可能存在幻觉的信息存储到数组中
        # {video_key:{}
        self.coaha_threshold = 0.05
        self.ah_threshold = 0.05
        self.oh_threshold = 0.05
        self.dis_threshold = 0.05
        self.hallucinated_video_info = {}
    ...
    def evaluate(self, prediction_dict):
        ...
        self.predicted_object_list = [str(x) for x in self.predicted_object_list if
                                      str(x) not in self.frequent_words_object]
        self.predicted_object_list = [str(x) for x in self.predicted_object_list if str(x) not in self.action_list]
        self.predicted_action_list = [str(x) for x in self.predicted_action_list if
                                      str(x) not in self.frequent_words_action]
        ...
        for k, v in prediction_dict.items():
            ...
            self.oh_list.append(oh_s)
            self.ah_list.append(ah_s)
        self.coaha_total = sum(self.coaha_list) / len(self.coaha_list)
        self.oh_total = sum(self.oh_list) / len(self.coaha_list)
        self.ah_total = sum(self.ah_list) / len(self.coaha_list)
    ...
    def evaluate_single(self, predicted, key):
        ...
        hallucinated_objects = [word for word in words if word in self.object_list] + [word for word in words if
                                                                                       word in self.predicted_object_list]
        ...
        hallucinated_actions = [word for word in words if word in self.action_list] + [word for word in words if
                                                                                       word in self.predicted_action_list]
        ...
        oh_distance = {"object_dict": self.object_dict[key]}
        ah_distance = {"action_dict": self.action_dict[key]}
        for word in hallucinated_objects:
            oh_1 = self._distance_calculation(word, self.object_dict[key])
            oh += oh_1
            oh_distance.update({word: oh_1})
        for word in hallucinated_actions:
            ah_1 = self._distance_calculation(word, self.action_dict[key])
            ah += ah_1
            ah_distance.update({word: ah_1})
        oh /= self.avg_length[key]
        ah /= self.avg_length[key]
        self.hallucinated_video_info.update({key: {"coaha": oh + ah, "oh": oh, "ah": ah, "hallucinated_objects": hallucinated_objects,"hallucinated_actions": hallucinated_actions, "words": words, "predicted": predicted, "oh_distance": oh_distance, "ah_distance": ah_distance}})
        return oh, ah
